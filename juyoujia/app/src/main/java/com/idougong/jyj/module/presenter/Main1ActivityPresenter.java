...
public class Main1ActivityPresenter extends BasePresenter<Main1ActivityContract.View> implements Main1ActivityContract.Presenter {
    @Inject
    ApiService apiService;

    @Inject
    public Main1ActivityPresenter() {
    ...
    public void getHomeMenu() {
        addSubscribe(apiService.getHomeMenu()
                .compose(RxUtil.<BaseResponseInfo<HomeMenu>>rxSchedulerHelper())
                .compose(RxUtil.<HomeMenu>handleResult())
                .doOnNext(new Consumer<HomeMenu>() {
                    @Override
                    public void accept(HomeMenu homeMenu) throws Exception {
                        LogUtils.d("success");
                    }
                })
                .subscribeWith(new CommonSubscriber<HomeMenu>(getView()) {
                    @Override
                    public void onNext(HomeMenu o) {
                        getView().setHomeMenu(o);
                    }
                }));
    }
